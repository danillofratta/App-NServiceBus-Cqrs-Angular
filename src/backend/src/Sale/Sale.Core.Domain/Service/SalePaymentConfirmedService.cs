...
{
    public class SalePaymentConfirmedService : BaseService
    {
        private readonly ISaleRepository _salerepository;
        private readonly SaleExistsValidator _SaleExistsValidator;

        public SalePaymentConfirmedService(ISaleRepository salerepository, SaleExistsValidator saleExistsValidator)
        {
        ...
        public async Task Process(Guid idsale)
        {
            try
            {
                ...
                var canBeCancelled = new SaleCanBePaymentConfirmedSpecification().IsSatisfiedBy(sale);
                if (!canBeCancelled)
                    throw new ValidationException("The sale cannot be confirmed at this stage.");
            ...
            catch (Exception ex)
            {
                this._ListError.Add
                    (
                        ...
                        {
                            Error = ex.Message,
                            Detail = "Error save payment not confirmed"
                        }
                    );
                throw ex;
            }
