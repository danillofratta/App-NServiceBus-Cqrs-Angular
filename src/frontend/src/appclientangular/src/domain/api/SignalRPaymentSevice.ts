...
export class SignalRPaymentSevice {
  private hubConnection: signalR.HubConnection;

  constructor() {
    this.hubConnection = new signalR.HubConnectionBuilder()
      //.withUrl('https://localhost:7091/notificationHub', {
      .withUrl(environment.ApiUrlPaymentSignal, {
  ...
  onGetListUpdated(callback: (dataList: any[]) => void): void {
    this.hubConnection.on('GetListPayment', callback);
    console.log('📢 Recebida atualização de pagamento:', callback);
  }
  ...
  receiveMessage(): Observable<string> {
    return new Observable<string>((observer) => {
      this.hubConnection.on('GetListPayment', (message: string) => {
